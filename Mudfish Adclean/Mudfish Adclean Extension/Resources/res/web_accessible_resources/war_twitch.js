...
(function() {
        ...
        `;
            ...
            this.onmessage = function(e) {
                if (e.data.key == 'ShowAdBlockBanner') {
                    ...
                    adBlockDiv.P.textContent = '미꾸라지 광고차단중...';
    ...
    async function getStreamForResolution(streamInfo, resolutionInfo, encodingsM3u8, fallbackStreamStr, playerType, realFetch) {
        ...
        if (playerType === 'proxy' || playerType === 'proxy2') {
        ...
        console.log('encodingsM3u8', encodingsM3u8)
        ...
        console.log(streamM3u8Url, 'streamM3u8Url');
        ...
        if (haveAdTags) {
            ...
            if (playerType === 'proxy' || playerType === 'proxy2') {
                try {
                    ...
                    if (playerType === 'proxy')
                        encodingsM3u8Response = await realFetch('https://api.ttv.lol/playlist/' + CurrentChannelName);
                    if (playerType === 'proxy2') {
                        encodingsM3u8Response = await realFetch('https://api.ttv.lol/playlist/' + CurrentChannelName + '.m3u8%3Fallow_source%3Dtrue', {
                            headers: {
